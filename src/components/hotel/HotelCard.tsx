...
export default function HotelCard({
  ...
  return (
    <article className="group rounded-xl bg-white shadow-sm hover:shadow-md transition-all duration-300 flex flex-col overflow-hidden border border-slate-100 h-full">
      <Link
        href={`/hotels/${slug}`}
        className="block relative w-full h-56 overflow-hidden bg-slate-100"
      >
        <Image
          src={
            safeCoverImage ||
      ...
      <div className="p-4 flex flex-col flex-1 justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-600 mt-1">
            Sleeps {capacity} Guests • Premium Serviced
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {amenities.slice(0, 3).map((amenity) => (
            <span
              key={amenity}
              className="px-2 py-1 rounded-md bg-slate-50 border border-slate-100 text-teal-900 font-bold text-[10px] flex items-center gap-1"
            >
              {amenityIconMap[amenity] || <Zap size={12} />}{" "}
              {amenityNameMap[amenity] || amenity}
            </span>
          ))}
          {amenities.length === 0 && (
        ...
        <div className="pt-2 mt-auto border-t border-slate-100 flex items-end justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <span className="font-serif text-lg text-teal-950 font-bold">
                {formattedPrice}
              </span>
