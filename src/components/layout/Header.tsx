        ...
        <div className="h-20 max-w-[1440px] mx-auto px-4 md:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-12">
            <Link href="/" className="flex items-center gap-2 group">
              ...
              <span
                className={`font-serif text-xl tracking-tight font-bold ${scrolled ? "text-teal-900" : "text-teal-950"}`}
              >
                Monarch Stay
              </span>
            </Link>
            ...
            <nav className="hidden lg:flex items-center gap-1 p-1">
              <Link
                href="/explore"
                className="px-4 py-2 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-all"
              >
                Explore Stays
              </Link>
              <Link
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {!session ? (
              <div className="flex items-center gap-3">
                <Link
                  href="/login"
                  className="text-sm font-semibold text-slate-700 hover:text-teal-900 transition-colors"
                >
                  Log in
                </Link>
                <Link
                  href="/register"
                  className="px-5 py-2.5 rounded-xl bg-teal-900 hover:bg-teal-800 text-white text-sm font-bold shadow-md hover:shadow-lg transition-all active:scale-95"
                >
                  Sign Up
                </Link>
              </div>
              ...
              <div className="flex items-center gap-3 pl-2">
                <Link
                  href="/profile"
                  className="flex items-center gap-2 p-1 pr-3 rounded-full border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all"
                >
                ...
                {session?.role === "ADMIN" && (
                  <Link
                    ...
                    className="text-xs font-bold px-3 py-1.5 rounded-md bg-slate-800 text-white hover:bg-slate-700"
                  >
                    Admin
                  </Link>
                )}
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-20 z-40 bg-white flex flex-col pt-6 px-6 md:hidden overflow-y-auto">
          <nav className="flex flex-col gap-6 text-lg font-medium text-slate-800">
            <Link
              href="/explore"
              className="flex items-center gap-3 hover:text-teal-900"
            ...
            {!session ? (
              <div className="flex flex-col gap-4">
                <Link
                  href="/login"
                  className="w-full py-3 text-center rounded-xl border border-slate-200 font-semibold"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Log in
                </Link>
                <Link
                  href="/register"
                  className="w-full py-3 text-center rounded-xl bg-teal-900 text-white font-bold"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Sign Up
                </Link>
              ...
              <div className="flex flex-col gap-4">
                <Link
                  href="/profile"
                  className="w-full py-3 text-center rounded-xl border border-slate-200 font-semibold flex items-center justify-center gap-2"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                ...
                <form action={logoutAction} className="w-full">
                  <button
                    type="submit"
                    className="w-full py-3 text-center rounded-xl bg-red-50 text-red-600 font-bold hover:bg-red-100"
                  >
                    Log out
                  </button>
                </form>
