...
export default async function UpcomingStay() {
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl text-slate-900 font-semibold">Upcoming Reservation</h2>
          </div>
        </div>
  ...
  const bookingRef = booking.id ? `#STN-${booking.id.substring(0, 5)}` : "#STN-8842-ABJ";
  ...
  if (booking.checkIn && booking.checkOut) {
    ...
    checkIn = inDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <span className="text-xs font-semibold text-teal-900 uppercase tracking-wider">Booking Ref: {bookingRef}</span>
      </div>
      ...
      <div className="bg-white rounded-xl overflow-hidden shadow-md flex flex-col border border-slate-100">
        <div className="relative w-full h-72">
          <Image 
            ...
            src={booking.hotel?.image || "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&q=80"}
            alt={hotelName}
            fill
          />
          ...
          <div className="absolute bottom-4 left-4 right-4 flex flex-col sm:flex-row sm:items-end justify-between gap-4 text-white z-10">
            ...
            <div className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-lg text-right border border-white/10">
              ...
              <div className="text-lg font-bold">{checkIn} – {checkOut}</div>
              <div className="text-xs text-orange-400 font-semibold">4 Nights • 2 Guests</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 flex flex-col gap-6 bg-white">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
            <div className="flex items-center gap-4">
              ...
              />
              <div>
                <h4 className="text-sm font-bold text-slate-900">Host: Tunde Alabi (Premier Host)</h4>
            ...
            <a href="https://wa.me" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg bg-[#25D366] text-white text-xs font-bold hover:opacity-90 shadow-sm transition-all w-full sm:w-auto justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button className="flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-100 text-slate-900 text-sm font-semibold transition-all" type="button">
            ...
            <button className="flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-100 text-orange-600 text-sm font-semibold transition-all" type="button">
