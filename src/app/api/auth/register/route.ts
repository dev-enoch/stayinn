...
export async function POST(req: Request) {
  try {
    ...
    if (!limit.success) {
      return NextResponse.json({ success: false, error: { code: 'RATE_LIMITED', message: 'Too many requests' } }, { status: 429, headers: { 'X-RateLimit-Remaining': '0' } });
    }
    ...
    if (existingPhone) {
      return NextResponse.json({ success: false, error: { code: 'PHONE_EXISTS', message: 'Phone number already registered' } }, { status: 409 });
    }
