...
export async function POST(req: Request) {
  try {
    ...
    if (!result.success) {
      return NextResponse.json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid credentials' } }, { status: 422 });
    }
    ...
    if (!user) {
      return NextResponse.json({ success: false, error: { code: 'INVALID_CREDENTIALS', message: 'Wrong identifier or password' } }, { status: 401 });
    }
