...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!hotel) {
      return NextResponse.json({ success: false, error: { code: 'NOT_FOUND', message: 'Hotel not found or not approved' } }, { status: 404 });
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        amenities: hotel.amenities.map((a: HotelAmenity) => ({ code: a.amenity, label: a.amenity })),
        roomTypes: hotel.roomTypes.map((rt: RoomType & { images: RoomImage[] }) => ({
          id: rt.id,
          name: rt.name,
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!hotel) {
      return NextResponse.json({ success: false, error: { code: 'NOT_FOUND', message: 'Hotel not found' } }, { status: 404 });
    }

    if (hotel.managerId !== payload.userId) {
      return NextResponse.json({ success: false, error: { code: 'FORBIDDEN', message: 'Not authorized to edit this hotel' } }, { status: 403 });
    }
    ...
    await prisma.$transaction([
      ...
      prisma.hotel.update({
        where: { id },
        data: {
          name, description, address, latitude, longitude, coverImage,
          amenities: amenities ? {
