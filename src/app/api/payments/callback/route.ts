  ...
  try {
    ...
    if (!reference) {
      return NextResponse.redirect(
        new URL("/?error=missing_reference", req.url),
      );
    }
    ...
    if (!payment) {
      return NextResponse.redirect(
        new URL("/?error=payment_not_found", req.url),
      );
    }
    ...
    if (verifyRes.data.status === "success") {
      ...
      if (payment.status === "SUCCESS" && booking.status === "PAID") {
        return NextResponse.redirect(
          new URL(`/bookings/${booking.id}?status=success`, req.url),
        );
      }
      ...
      return NextResponse.redirect(
        new URL(`/bookings/${booking.id}?status=failed`, req.url),
      );
    }
  } catch (error) {
