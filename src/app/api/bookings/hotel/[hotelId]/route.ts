...
export async function GET(req: Request, { params }: { params: Promise<{ hotelId: string }> }) {
  try {
    ...
    if (!hotel || (hotel.managerId !== payload.userId && payload.role !== 'ADMIN')) {
    ...
    const items = bookings.map((b: Booking & { roomType: { name: string }, user: { fullName: string } }) => ({
      id: b.id,
