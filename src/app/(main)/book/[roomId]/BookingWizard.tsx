...
import { ArrowLeft, ArrowRight, Check, Users, Calendar as CalendarIcon, ShieldCheck } from "lucide-react";

import { createBookingAndPay } from "@/app/actions/booking";

export default function BookingWizard({ room, user }: { room: any, user: any }) {
  ...
  const commission = total * 0.10; // 10% deducted from hotel payout
          ...
          {[1, 2, 3, 4].map(num => (
            <div 
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${
                step >= num ? "bg-green-600 text-white shadow-md" : "bg-white text-gray-300 border-2 border-gray-100"
              }`}
            >
        ...
        <div className="w-full md:w-3/5">
          {step === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-3xl font-bold tracking-tight text-gray-900">Who's coming?</h2>
              <div className="p-6 border border-gray-200 rounded-3xl flex justify-between items-center bg-gray-50/50">
                <div>
                  ...
                  <p className="text-gray-500">Maximum capacity: {room.capacity}</p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-2xl font-bold w-6 text-center">{guests}</span>
                  <button 
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-4 mb-8">
                <button onClick={handleBack} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><ArrowLeft size={24} /></button>
                <h2 className="text-3xl font-bold tracking-tight text-gray-900">When are you staying?</h2>
              </div>
              
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="font-bold text-sm text-gray-700 uppercase tracking-wide">Check-in</label>
                  <input 
                    type="date" 
                    value={checkIn}
                ...
                <div className="space-y-2">
                  <label className="font-bold text-sm text-gray-700 uppercase tracking-wide">Check-out</label>
                  <input 
                    type="date" 
                    value={checkOut}
          ...
          {step === 3 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-4 mb-8">
                <button onClick={handleBack} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><ArrowLeft size={24} /></button>
                <h2 className="text-3xl font-bold tracking-tight text-gray-900">Review your trip</h2>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center py-4 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-gray-500">{new Date(checkIn).toLocaleDateString()} – {new Date(checkOut).toLocaleDateString()}</p>
                  </div>
                  <button onClick={() => setStep(2)} className="text-green-600 font-semibold underline">Edit</button>
                </div>
                <div className="flex justify-between items-center py-4 border-b border-gray-100">
                  <div>
                  ...
                  <button onClick={() => setStep(1)} className="text-green-600 font-semibold underline">Edit</button>
                </div>
              </div>
          ...
          {step === 4 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-4 mb-8">
                <button onClick={handleBack} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><ArrowLeft size={24} /></button>
                <h2 className="text-3xl font-bold tracking-tight text-gray-900">Secure Checkout</h2>
              </div>

              <div className="bg-green-50 p-6 rounded-3xl flex items-start gap-4">
                <ShieldCheck className="text-green-600 flex-shrink-0 mt-1" size={24} />
                <p className="text-gray-700 font-medium leading-relaxed">
                  You will be securely redirected to Paystack to complete your payment. Stayinn never stores your card details.
                </p>
              </div>
          ...
          <div className="border border-gray-200 rounded-3xl p-6 sticky top-28 bg-white shadow-sm">
            <div className="flex gap-4 mb-6">
              ...
              <div className="flex flex-col justify-center">
                <p className="text-sm text-gray-500 uppercase tracking-widest font-bold mb-1">{room.hotelName}</p>
            ...
            <h4 className="font-bold text-gray-900 text-lg mb-4">Price details</h4>
            
            <div className="space-y-4 text-gray-600">
              <div className="flex justify-between items-center py-2">
                <span>{formatPrice(room.pricePerNight)} x {nights} night{nights !== 1 ? 's' : ''}</span>
            ...
            <div className="flex justify-between items-center">
              <span className="font-bold text-gray-900 text-lg">Total (NGN)</span>
