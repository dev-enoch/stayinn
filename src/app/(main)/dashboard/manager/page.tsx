...
export default async function ManagerDashboardPage() {
  ...
  const hotel = dbHotel
    ? {
        ...
        totalEarnings: dbHotel.bookings.reduce(
          (sum, b) => sum + b.totalAmount * 0.9,
          0,
        ),
        pendingCheckins: dbHotel.bookings.filter((b) => b.status === "PAID")
          .length,
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-28 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link
            href="/dashboard/manager/setup"
            className="bg-gray-900 text-white px-8 py-4 rounded-full font-bold hover:bg-green-600 transition-colors"
          >
            Register Property
          </Link>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Status:{" "}
              <span
                className={`font-bold ${hotel.status === "APPROVED" ? "text-green-600" : "text-orange-500"}`}
              >
                {hotel.status}
              </span>
            </p>
          ...
          <Link
            href="/dashboard/manager/scanner"
            className="bg-gray-900 text-white px-6 py-3 rounded-full font-bold hover:bg-green-600 transition-colors flex items-center gap-2"
          >
            <QrCode size={20} />
            Scan Check-in QR
          </Link>
