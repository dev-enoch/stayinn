    ...
    <div className="flex flex-col w-full bg-slate-50 min-h-screen pt-20">
      <section className="relative w-full pt-40 pb-16 overflow-hidden bg-slate-900 text-white">
        ...
        <div className="max-w-[1280px] mx-auto px-4 md:px-12 relative z-10">
          <Link
            href="/cities"
            className="inline-flex items-center gap-2 text-teal-200 hover:text-white transition-colors mb-6 text-sm font-bold"
          >
          ...
          <p className="text-lg text-teal-100/80 leading-relaxed max-w-xl">
            {totalHotels} properties found in this location. Explore our curated
            selection of premium serviced apartments and luxury stays.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-[1280px] mx-auto px-4 md:px-12">
          {formattedHotels.length === 0 ? (
            <div className="w-full bg-white rounded-2xl p-12 text-center shadow-sm border border-slate-100 flex flex-col items-center justify-center">
              ...
              <p className="text-slate-500 max-w-md mx-auto">
                We couldn't find any approved properties in {cityName} matching
                your criteria.
              </p>
            </div>
          ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-4 mt-16 border-t border-slate-200 pt-8">
              {currentPage > 1 ? (
                <Link
                  href={`/cities/${resolvedParams.cityname}?page=${currentPage - 1}`}
                  className="px-6 py-3 rounded-xl bg-white border border-slate-200 text-slate-900 font-bold hover:bg-slate-50 transition-colors shadow-sm"
                >
                  Previous
                </Link>
              ) : (
                <button
                  disabled
                  className="px-6 py-3 rounded-xl bg-slate-100 border border-slate-200 text-slate-400 font-bold cursor-not-allowed"
                >
                  Previous
                </button>
              )}
