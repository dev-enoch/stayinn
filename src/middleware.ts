  ...
  if (isProtectedApi) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return addSecurityHeaders(
        NextResponse.json({ success: false, error: { code: 'UNAUTHORIZED', message: 'Missing token' } }, { status: 401 })
      );
    }
    ...
    if (!payload) {
      return addSecurityHeaders(
        NextResponse.json({ success: false, error: { code: 'TOKEN_INVALID', message: 'Invalid or expired token' } }, { status: 401 })
      );
    }
    ...
    if (pathname.startsWith('/api/admin') && payload.role !== 'ADMIN') {
      return addSecurityHeaders(
        NextResponse.json({ success: false, error: { code: 'FORBIDDEN', message: 'Admin access required' } }, { status: 403 })
      );
    }
  }
